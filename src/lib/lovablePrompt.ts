...
const publicPageContract = [
  'Le retour Lovable doit contenir une section obligatoire E. PublicPageConfig.',
...
export function generateLovablePrompt(input: LovablePromptInput): LovablePrompt {
  const sections = [
    ...
    createSection('FORMAT DE SORTIE ATTENDU', [
      ...
      'C. Un pack visuel minimal : logo trouve ou etat "logo non disponible", palette de couleurs, typographies proposees ou detectees, URLs ou references des photos publiques de home selectionnees.',
      'D. Une courte section "Capacités non supportées détectées" listant uniquement les elements visibles de la demo que le moteur actuel ne sait pas reproduire.',
      'E. Une section PublicPageConfig complete avec publicPage.sections et publicPage.imageRoles.',
